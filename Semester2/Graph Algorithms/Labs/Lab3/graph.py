...
def minimum_cost_path(graph, start_vertex, end_vertex):
    """
    Function to find the minimum cost path between 2 vertices
    :param graph: graph
    :param start_vertex: start vertex
    :param end_vertex: end vertex
    :return:the distance and the parsed vertices
    """
    number_of_vertices = graph.number_vertices
    number_of_edges = graph.number_edges
    ...
    for k in range(1, number_of_edges+1):
        for vertex in graph.parse_vertices():
            ...
            for prev_vertex in graph.parse_in(vertex):
