...
public class Matrix {
    ...
    public static Matrix readFromFile() throws IOException {
        ...
        int freeI = -1, freeJ = -1;
        Scanner scanner = new Scanner(new BufferedReader(new FileReader("src/Threads/input.in")));
    ...
    // the sum where each term is how far is a value from its correct position with both vertical and horizontal measures
    ...
    // go from the final state in each previous state, append them to the result and reverse the result in order to start with the initial state
    // and end with the final one.
    @Override
