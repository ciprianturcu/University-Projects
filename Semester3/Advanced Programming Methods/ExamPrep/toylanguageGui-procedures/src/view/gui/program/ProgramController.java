...
public class ProgramController {
    ...
    public void setController(Controller controller) throws ADTException {
    ...
    private void populate() throws ADTException {
    ...
    @FXML
    private void changeProgramState(MouseEvent event) throws ADTException {
        populateExecutionStackListView();
        populateSymbolTableView();
    }
    ...
    public void populateProcedureTableView()
    {
        InterfaceProcedureTable procedureTable = Objects.requireNonNull(getCurrentProgramState()).getProcedureTable();
        List<Map.Entry<String, javafx.util.Pair<List<String>, IStmt>>> procedureTableList = new ArrayList<>();
        for(Map.Entry<String, javafx.util.Pair<List<String>, IStmt>> entry : procedureTable.getContent().entrySet())
        {
            Map.Entry<String, javafx.util.Pair<List<String>, IStmt>> entry1 = new AbstractMap.SimpleEntry<>(entry.getKey(), entry.getValue());
            procedureTableList.add(entry1);
        }
        procTableView.setItems(FXCollections.observableArrayList(procedureTableList));
        procTableView.refresh();
    }

    private void populateHeapTableView() {
        PrgState programState = getCurrentProgramState();
        InterfaceHeap heap = Objects.requireNonNull(programState).getHeap();
    ...
    private void populateSymbolTableView() throws ADTException {
        PrgState programState = getCurrentProgramState();
        InterfaceDictionary<String, Value> symbolTable = Objects.requireNonNull(programState).getTopSymTable();
    ...
    private void populateExecutionStackListView() {
        ...
        if (programState != null)
            for (IStmt statement: programState.getExeStack().getAllReversed()) {
    ...
    private void runOneStep(MouseEvent mouseEvent) {
        if (controller != null) {
            ...
            } catch (InterruptedException | ADTException e) {
                Alert alert = new Alert(Alert.AlertType.ERROR);
