...
public class PrgState {
    ...
    private InterfaceSemaphoreTable semaphoreTable;
    private int id;
    private static int lastId =0;
    private IStmt originalProgram;


    public PrgState(InterfaceStack<IStmt> exeStack, InterfaceDictionary<String, Value> symbolTable, InterfaceList<Value> out, IStmt program, InterfaceDictionary<String, BufferedReader> fileTable, InterfaceHeap heap, InterfaceSemaphoreTable semaphoreTable) {
    ...
    public void setSemaphoreTable(InterfaceSemaphoreTable semaphoreTable) {
        this.semaphoreTable = semaphoreTable;
    }
    ...
    public String semaphoreToString() throws ADTException
    {
        StringBuilder semaphoreToStringBuilder = new StringBuilder();
        for(int key : semaphoreTable.getSemaphoreTable().keySet())
                semaphoreToStringBuilder.append(String.format("%s -> (%d, %s)\n", key, semaphoreTable.get(key).getKey(),semaphoreTable.get(key).getValue()));
        return semaphoreToStringBuilder.toString();
    }
