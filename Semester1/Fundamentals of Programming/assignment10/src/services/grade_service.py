from src.my_stuff.my_sort import my_sort
from src.my_stuff.my_filter import my_filter
...
class GradeService:
    ...
    def generate_ungraded_assignments(self):
        ...
        index = 0
        for i in range(20):
            grade = Grade((assignment_list[index].id, student_list[index].id), "0")
    ...
    def get_assignments_descending_order(self):
        ...
        """
        unordered_list = self.__data.values

        def comparison_function(first_element, second_element):
            if first_element.grade <= second_element.grade:
                return True
            else:
                return False

        order_list = my_sort(unordered_list, comparison_function)
    ...
    def get_late_assignments(self):
        ...
        def filter_function(element):
            if datetime.datetime.strptime(element.deadline, "%d/%m/%Y")<datetime.datetime.now():
                return True
            else:
                return False
        ...
        assignment_data = my_filter(assignment_data, filter_function)
        result_list = list()
    ...
    def get_students_top(self):
        ...
        def filter_function(element_grade):
            if element_grade == '0':
                return False
            else:
                return True
        ...
        def comparison_function(first_element, second_element):
            if first_element.grade_value <= second_element.grade_value:
                return True
            else:
                return False

        result_list = my_sort(unordered_list, comparison_function)
        return result_list
