function x=jacobiMethodv2(A,b,era,err,Nmax)
[~, n] = size(A);
...
xc = xp;
for k=1:Nmax
    xc(1) = 1/A(1,1) * (b(1) - A(1,2) * xp(2) - A(1, n) * xp(n));
    for i=2:(n/2-1)
        xc(i) = 1/A(i,i) * (b(i) - A(i,i-1) * xp(i-1) - A(i,i+1) * xp(i+1) - A(i, n-i+1) * xp(n-i+1));
    end
    ...
    xc(n) = 1 / A(n,n) * (b(n) - A(n,n-1) * xp(n-1) - A(n, 1) * xp(1));
